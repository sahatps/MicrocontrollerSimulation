...
exports.convertBfarmMacroToCpp = exports.hasBfarmMacroMarkers = void 0;
...
function hasBfarmMacroMarkers(code) {
    return BFARM_MARKER_REGEX.test(code);
}
exports.hasBfarmMacroMarkers = hasBfarmMacroMarkers;
...
function getFunc(src, name) {
    const re = new RegExp(String.raw `(void\s+${name}\s*\(\s*\)\s*\{)([\s\S]*?)(\n[ \t]*\})`, 'm');
...
function stripEmptyLines(source) {
    return source.replace(/\n{3,}/g, '\n\n').trim();
}
function normalizeLineForCompare(line) {
    return line.replace(/\s+/g, ' ').trim();
}
function dedupeByLine(source) {
...
function dedupeIncludes(source) {
    const lines = source.replace(/\r\n/g, '\n').split('\n');
    const seen = new Set();
    const out = [];
    for (const line of lines) {
...
function convertBfarmMacroToCpp(input) {
    var _a, _b;
    ...
    const existingSetup = ((_a = setupPart === null || setupPart === void 0 ? void 0 : setupPart.body) !== null && _a !== void 0 ? _a : '').trim();
    const existingLoop = ((_b = loopPart === null || loopPart === void 0 ? void 0 : loopPart.body) !== null && _b !== void 0 ? _b : '').trim();
    const newSetupBody = stripEmptyLines([blocks.SETUP, blocks.BLOCKSETUP, existingSetup].filter(Boolean).join('\n\n'));
    const newLoopBody = stripEmptyLines([blocks.LOOP_EXT_CODE, existingLoop].filter(Boolean).join('\n\n'));
...
exports.convertBfarmMacroToCpp = convertBfarmMacroToCpp;
